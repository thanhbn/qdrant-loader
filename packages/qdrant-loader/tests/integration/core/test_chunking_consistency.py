...
"""
    ...
    def test_both_strategies_use_character_based_chunking(
        ...
        with patch(
            ...
            with (
                ...
                assert (
                    len(default_chunks) > 1
                ), "Default strategy should create multiple chunks"
                assert (
                    len(markdown_chunks) > 1
                ), "Markdown strategy should create multiple chunks"
                ...
                for chunk in default_chunks:
                    ...
                    assert (
                        len(chunk.content) <= max_size + 100
                ...
                assert (
                    expected_chunks * 0.5 <= len(default_chunks) <= expected_chunks * 2
                ...
                assert (
                    len(markdown_chunks) >= 2
                ), "Markdown strategy should create at least 2 chunks"
    ...
    def test_character_size_consistency(self, mock_settings):
        ...
        for chunk_size in [100, 300, 500]:
            ...
            with (
                ...
                assert (
                    len(default_chunks) > 0
                ), "Default strategy should create at least one chunk"

                for chunk in default_chunks:
                    # Allow tolerance for overlap and boundary detection
                    assert (
                        len(chunk.content) <= chunk_size + 100
                ...
                assert (
                    len(default_chunks) >= expected_chunks * 0.5
    ...
    def test_tokenizer_boundary_detection_still_character_based(self, mock_settings):
        ...
        test_content = (
            "This is a sentence with clear word boundaries. " * 10
        )  # ~460 characters
