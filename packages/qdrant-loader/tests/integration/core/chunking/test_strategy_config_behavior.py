...
class TestMarkdownStrategyConfigBehavior:
    ...
    def test_min_content_length_for_nlp_affects_processing(
        ...
        self.create_test_document(short_content)
        self.create_test_document(medium_content)
        ...
        short_skip_nlp = (
            len(short_content) < 100
            or len(short_content.split()) < 20
            or short_content.count("\n") < 3
        )
        medium_skip_nlp = (
            len(medium_content) < 100
            or len(medium_content.split()) < 20
            or medium_content.count("\n") < 3
        )
    ...
    def test_min_word_count_for_nlp_affects_processing(self, mock_semantic_analyzer):
        ...
        MarkdownChunkingStrategy(settings)
        ...
        MarkdownChunkingStrategy(low_word_settings)
    ...
    def test_estimation_buffer_affects_chunk_estimation(self):
        ...
        base_estimate = (
            len(content) // default_settings.global_config.chunking.chunk_size
        )
        ...
        high_buffer_settings = self.create_settings_with_markdown_config(
    ...
    def test_words_per_minute_affects_reading_time(self):
        ...
        assert (
            enriched_meta["content_type_analysis"]["estimated_read_time"]
            == default_reading_time
        )

        # Test with faster reading speed (400 WPM)
        fast_settings = self.create_settings_with_markdown_config(
            words_per_minute_reading=400
        )
        ...
        assert (
            fast_meta["content_type_analysis"]["estimated_read_time"]
            == fast_reading_time
        )
        assert (
            fast_meta["content_type_analysis"]["estimated_read_time"]
...
class TestChunkingSizeConfigBehavior:
    ...
    def create_settings_with_chunking_config(
        self, chunk_size=1500, chunk_overlap=200, max_chunks=500
    ):
    ...
    def test_chunk_size_affects_chunk_count(self):
        ...
        small_chunk_settings = self.create_settings_with_chunking_config(
            chunk_size=500, chunk_overlap=100
        )
        small_strategy = DefaultChunkingStrategy(small_chunk_settings)

        # Test with large chunk size
        large_chunk_settings = self.create_settings_with_chunking_config(
        ...
        for chunk in small_chunks:
            ...
            assert (
                len(chunk.content)
                <= small_chunk_settings.global_config.chunking.chunk_size + 200
            )

        for chunk in large_chunks:
            assert (
                len(chunk.content)
                <= large_chunk_settings.global_config.chunking.chunk_size + 200
            )
    ...
    def test_max_chunks_per_document_limits_output(self):
        ...
        limited_settings = self.create_settings_with_chunking_config(
            chunk_size=200, chunk_overlap=50, max_chunks=5  # Very low limit
        )
        ...
        assert (
            len(chunks)
            <= limited_settings.global_config.chunking.max_chunks_per_document
        )
...
class TestConfigurationIntegrationBehavior:
    ...
    def test_yaml_config_propagates_to_strategy_behavior(self):
        ...
        assert (
            strategy.settings.global_config.chunking.strategies.markdown.min_section_size
            == 300
        )
        assert (
            strategy.settings.global_config.chunking.strategies.markdown.max_workers
            == 6
        )

        # Verify the values are actually used in components
        assert (
            strategy.section_splitter.settings.global_config.chunking.chunk_size == 800
        )
    ...
    def test_strategy_selection_uses_correct_config(self):
        ...
        default_config = (
            default_strategy.settings.global_config.chunking.strategies.default
        )
        markdown_config = (
            markdown_strategy.settings.global_config.chunking.strategies.markdown
        )
