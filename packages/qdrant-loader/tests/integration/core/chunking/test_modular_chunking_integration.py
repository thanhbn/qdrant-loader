...
class TestModularChunkingIntegration:
    ...
    def test_base_components_integration(self, real_settings):
        ...
        from qdrant_loader.core.chunking.strategy.base.chunk_processor import (
            BaseChunkProcessor,
        )
        ...
        class TestMetadataExtractor(BaseMetadataExtractor):
            def extract_hierarchical_metadata(
                self, content: str, chunk_metadata: dict, document
            ):
                ...
                metadata.update(
                    {
                        ...
                        "keyword_density": self.extract_keyword_density(
                            content, top_n=5
                        ),
                    }
                )
        ...
        for section in sections:
            ...
            enriched_sections.append(
                {"content": section["content"], "metadata": enriched_metadata}
            )
    ...
    def test_configuration_driven_behavior(self, sample_documents):
        ...
        for chunk in small_chunks:
            assert (
                len(chunk)
                <= small_config.chunking.chunk_size
                + small_config.chunking.chunk_overlap
            )

        for chunk in large_chunks:
            assert (
                len(chunk)
                <= large_config.chunking.chunk_size
                + large_config.chunking.chunk_overlap
            )
