...
class TestLocalFileFileProcessor:
    ...
    def create_test_file(self, relative_path: str, content: str = "test content", size: int = None):
    ...
    def test_should_process_file_file_conversion_enabled_supported(self):
        ...
        file_path = self.create_test_file("document.pdf")  # Not in file_types but convertible
        
        result = self.processor.should_process_file(file_path)
        
        assert result is True
        self.mock_file_detector.is_supported_for_conversion.assert_called_once_with(file_path)
    ...
    def test_should_process_file_pattern_edge_cases(self):
        ...
        self.base_config.exclude_paths = ["/temp/**"]  # Leading slash should be stripped
        file_path = self.create_test_file("temp/file.txt")
    ...
    def test_file_logging_debug_calls(self):
        ...
        with patch.object(self.processor.logger, 'debug') as mock_debug:
            ...
            assert any("Checking if file should be processed" in call for call in debug_calls)
