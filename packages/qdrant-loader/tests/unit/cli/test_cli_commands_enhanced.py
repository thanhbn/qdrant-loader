    ...
    def test_load_config_with_explicit_path_success(self):
        ...
        try:
            with patch('qdrant_loader.config.initialize_config') as mock_init:
                ...
                mock_init.assert_called_once_with(config_path, None, skip_validation=False)
        finally:
            config_path.unlink()
        ...
        with tempfile.TemporaryDirectory() as temp_dir:
            ...
            try:
                ...
                with patch('qdrant_loader.config.initialize_config') as mock_init:
                    ...
                    mock_init.assert_called_once_with(default_config, None, skip_validation=False)
            finally:
                os.chdir(str(original_cwd))
    ...
    def test_load_config_with_env_path(self):
        ...
        try:
            with patch('qdrant_loader.config.initialize_config') as mock_init:
                ...
                mock_init.assert_called_once_with(config_path, env_path, skip_validation=False)
        finally:
            config_path.unlink()
            env_path.unlink()
    ...
    def test_load_config_skip_validation(self):
        ...
        try:
            with patch('qdrant_loader.config.initialize_config') as mock_init:
                ...
                mock_init.assert_called_once_with(config_path, None, skip_validation=True)
        finally:
            config_path.unlink()
        ...
        with tempfile.TemporaryDirectory() as temp_dir:
            ...
            try:
                with patch('qdrant_loader.config.initialize_config') as mock_init:
                    ...
                    error = DatabaseDirectoryError(f"Database directory does not exist: {db_path}")
                    error.path = db_path  # Set path attribute
                    mock_init.side_effect = [error, None]
                    
                    with patch('qdrant_loader.cli.cli._create_database_directory', return_value=True) as mock_create:
                        _load_config(config_path)
        ...
        with tempfile.TemporaryDirectory() as temp_dir:
            ...
            try:
                with patch('qdrant_loader.config.initialize_config') as mock_init:
                    ...
                    with patch('qdrant_loader.cli.cli._create_database_directory', return_value=False):
                        with pytest.raises(ClickException) as exc_info:
                            _load_config(config_path)
                        
                        assert "Database directory creation declined" in str(exc_info.value)
            finally:
                config_path.unlink()
...
class TestSetupWorkspace:
    ...
    def test_setup_workspace_success(self):
        ...
        with tempfile.TemporaryDirectory() as temp_dir:
            ...
            with patch('qdrant_loader.config.workspace.create_workspace_structure') as mock_create:
                with patch('qdrant_loader.config.workspace.setup_workspace') as mock_setup:
                    ...
                    with patch('qdrant_loader.cli.cli._get_logger') as mock_logger:
                        ...
                        assert len(logger_calls) == 3  # workspace, env_path, config_path
...
class TestGetLogger:
    ...
    def test_get_logger_lazy_initialization(self):
        ...
        with patch('qdrant_loader.utils.logging.LoggingConfig.get_logger') as mock_get_logger:
            mock_logger = Mock()
...
class TestCLICommands:
    ...
    def test_cli_main_command_with_log_level(self):
        ...
        with patch('qdrant_loader.cli.cli._setup_logging') as mock_setup_logging:
