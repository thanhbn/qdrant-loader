...
class TestProjectCommandsCoverage:
    ...
    async def test_get_project_document_count_success(self):
        ...
        mock_state_manager._session_factory.return_value.__aenter__ = AsyncMock(
            return_value=mock_session
        )
        mock_state_manager._session_factory.return_value.__aexit__ = AsyncMock(
            return_value=None
        )
    ...
    async def test_get_project_latest_ingestion_success(self):
        ...
        timestamp = await _get_project_latest_ingestion(
            mock_state_manager, "test_project"
        )
    ...
    async def test_get_project_latest_ingestion_none_result(self):
        ...
        mock_result.scalar_one_or_none.return_value = (
            None  # This triggers line 72 condition
        )
        mock_session.execute.return_value = mock_result
