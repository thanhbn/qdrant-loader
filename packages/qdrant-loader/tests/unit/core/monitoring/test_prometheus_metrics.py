...
class TestPrometheusMetrics:
    ...
    def setup_method(self):
        ...
        try:
            ...
            for hist in [
                prometheus_metrics.CHUNKING_DURATION,
    ...
    def test_metrics_definitions(self):
        ...
        assert (
            prometheus_metrics.INGESTED_DOCUMENTS._name == "qdrant_ingested_documents"
        )
        assert (
            "Total number of documents ingested"
            in prometheus_metrics.INGESTED_DOCUMENTS._documentation
        )

        # Test Histogram metrics
        assert (
            prometheus_metrics.CHUNKING_DURATION._name
            == "qdrant_chunking_duration_seconds"
        )
        assert (
            "Time spent chunking documents"
            in prometheus_metrics.CHUNKING_DURATION._documentation
        )

        assert (
            prometheus_metrics.EMBEDDING_DURATION._name
            == "qdrant_embedding_duration_seconds"
        )
        assert (
            "Time spent embedding chunks"
            in prometheus_metrics.EMBEDDING_DURATION._documentation
        )

        assert (
            prometheus_metrics.UPSERT_DURATION._name == "qdrant_upsert_duration_seconds"
        )
        assert (
            "Time spent upserting to Qdrant"
            in prometheus_metrics.UPSERT_DURATION._documentation
        )

        # Test Gauge metrics
        assert prometheus_metrics.CHUNK_QUEUE_SIZE._name == "qdrant_chunk_queue_size"
        assert (
            "Current size of the chunk queue"
            in prometheus_metrics.CHUNK_QUEUE_SIZE._documentation
        )

        assert prometheus_metrics.EMBED_QUEUE_SIZE._name == "qdrant_embed_queue_size"
        assert (
            "Current size of the embedding queue"
            in prometheus_metrics.EMBED_QUEUE_SIZE._documentation
        )
    ...
    def test_metrics_thread_safety(self):
        ...
        assert (
            prometheus_metrics.INGESTED_DOCUMENTS._value._value == 500
    ...
    def test_all_metrics_are_registered(self):
        ...
        for expected_metric in expected_metrics:
            assert any(
                expected_metric in name for name in metric_names
    ...
    def test_logging_integration(self):
        ...
        assert (
            prometheus_metrics.logger.name
            == "qdrant_loader.core.monitoring.prometheus_metrics"
        )
