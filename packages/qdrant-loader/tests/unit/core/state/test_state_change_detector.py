...
from qdrant_loader.core.state.state_change_detector import (
    DocumentState,
    StateChangeDetector,
)
...
class TestStateChangeDetector:
    ...
    @pytest.mark.asyncio
    async def test_detect_changes_not_initialized(
        self, mock_state_manager, sample_documents, filtered_config
    ):
    ...
    @pytest.mark.asyncio
    async def test_detect_changes_new_documents(
        self, mock_state_manager, sample_documents, filtered_config
    ):
        ...
        async with detector:
            with patch.object(detector, "logger") as mock_logger:
                result = await detector.detect_changes(
                    sample_documents, filtered_config
                )
    ...
    @pytest.mark.asyncio
    async def test_detect_changes_updated_documents(
        self, mock_state_manager, sample_documents, filtered_config
    ):
        ...
        previous_record = DocumentStateRecord(
            ...
            updated_at=datetime(2022, 12, 31, tzinfo=UTC),  # Earlier date
        )
    ...
    @pytest.mark.asyncio
    async def test_detect_changes_deleted_documents(
        self, mock_state_manager, sample_documents, filtered_config
    ):
    ...
    def test_get_document_state_error(self, mock_state_manager):
        ...
        invalid_doc.updated_at = datetime.now(UTC)
    ...
    def test_normalize_url(self, mock_state_manager):
        ...
        assert (
            detector._normalize_url("http://example.com/") == "http%3A%2F%2Fexample.com"
        )
        assert (
            detector._normalize_url("http://example.com/path")
    ...
    def test_generate_uri(self, mock_state_manager):
        ...
        uri = detector._generate_uri("http://example.com/doc", "repo1", "git", "doc_id")
    ...
    @pytest.mark.asyncio
    async def test_detect_changes_empty_documents(
        self, mock_state_manager, filtered_config
    ):
    ...
    @pytest.mark.asyncio
    async def test_detect_changes_logging(
        self, mock_state_manager, sample_documents, filtered_config
    ):
