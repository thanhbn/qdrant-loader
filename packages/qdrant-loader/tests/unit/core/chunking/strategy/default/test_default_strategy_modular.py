...
class TestDefaultChunkingStrategyModular:
    ...
    def test_initialization_logging(self, settings):
        ...
        with (
            ...
            DefaultChunkingStrategy(settings)
    ...
    def test_chunk_document_basic_functionality(self, strategy, sample_document):
        ...
        strategy.document_parser.parse_document_structure = Mock(
            return_value={
                "paragraph_count": 2,
                "sentence_count": 3,
        ...
        strategy.section_splitter.split_sections = Mock(
            return_value=[
                {
                    "content": "This is a sample document for testing. It has multiple sentences and paragraphs.",
        ...
        strategy.metadata_extractor.extract_hierarchical_metadata = Mock(
            side_effect=lambda content, meta, doc: {
                **meta,
        ...
        strategy.chunk_processor.should_skip_semantic_analysis = Mock(
            return_value=False
        )
        strategy.chunk_processor.create_chunk_document = Mock(
            side_effect=lambda **kwargs: Document(
