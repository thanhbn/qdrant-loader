...
class TestTextDocumentParser:
    ...
    def test_extract_section_metadata(self, parser):
        ...
        expected_keys = {
            "section_type", "length", "word_count", "sentence_count",
            "has_formatting", "is_list_item", "is_numbered_item", "content_type"
        }
    ...
    def test_is_new_paragraph_start_list_items(self, parser):
        ...
        assert parser._is_new_paragraph_start("1. Numbered item", current_paragraph) is True
        
        # Regular text should not
        assert parser._is_new_paragraph_start("Regular text", current_paragraph) is False
    ...
    def test_is_new_paragraph_start_headers(self, parser):
        ...
        assert parser._is_new_paragraph_start("1. Section Title", current_paragraph) is True
    ...
    def test_classify_content_type(self, parser):
        ...
        assert parser._classify_content_type("1. Section Header") == "numbered_item"  # Could be header too
        assert parser._classify_content_type("Short") == "fragment"
        assert parser._classify_content_type("Title Without Period") == "header"  # This looks like a header
        assert parser._classify_content_type("This is a proper sentence.") == "paragraph"
    ...
    def test_looks_like_header(self, parser):
        ...
        assert parser._looks_like_header("This is way too long to be a reasonable header title for any document") is False
    ...
    def test_has_significant_indentation_change(self, parser):
        ...
        assert parser._has_significant_indentation_change(prev_line, current_line) is False
        
        # Significant change (>4 spaces)
        prev_line = "Regular line"
        current_line = "        Heavily indented"
        assert parser._has_significant_indentation_change(prev_line, current_line) is True
