...
class TestCodeChunkingStrategyModern:
    ...
    def test_chunk_javascript_document_success(
        self, mock_settings, sample_javascript_document
    ):
    ...
    def test_chunk_document_preserves_code_structure(
        self, mock_settings, sample_python_document
    ):
    ...
    def test_chunk_document_max_chunks_limit(self, mock_settings):
        ...
        with patch("qdrant_loader.core.text_processing.text_processor.TextProcessor"):
            ...
            assert (
                len(chunks) <= max_allowed or len(chunks) <= max_allowed + 100
            )  # Allow some tolerance for fallback
    ...
    def test_modular_components_integration(
        self, mock_settings, sample_python_document
    ):
