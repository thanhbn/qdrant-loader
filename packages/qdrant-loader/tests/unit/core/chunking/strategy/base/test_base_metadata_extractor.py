...
class TestBaseMetadataExtractor:
    ...
    def test_extract_cross_references_figure_references(self, extractor):
        ...
        content = (
            "As shown in Figure 2.1, the data indicates trends. Table A shows results."
        )
    ...
    def test_create_breadcrumb_metadata(self, extractor):
        ...
        breadcrumb = extractor.create_breadcrumb_metadata(
            current_section, parent_sections
        )

        assert breadcrumb["breadcrumb_path"] == [
            "Chapter 1",
            "Introduction",
            "Section 1.2",
        ]
        assert (
            breadcrumb["breadcrumb_string"] == "Chapter 1 > Introduction > Section 1.2"
        )
    ...
    def test_concrete_implementation_methods(self, extractor, sample_document):
        ...
        assert (
            enriched_metadata["existing_key"] == "existing_value"
        )  # Preserves existing
