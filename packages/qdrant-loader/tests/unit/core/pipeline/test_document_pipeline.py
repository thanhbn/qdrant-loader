...
class TestDocumentPipeline:
    ...
    @pytest.mark.asyncio
    async def test_process_documents_success(self, document_pipeline, mock_workers, sample_documents):
        ...
        upsert_worker.process_embedded_chunks.assert_called_once_with(embedded_chunks_iter)
    ...
    @pytest.mark.asyncio
    async def test_process_documents_with_errors(self, document_pipeline, mock_workers, sample_documents):
    ...
    @pytest.mark.asyncio
    async def test_process_documents_timeout(self, document_pipeline, mock_workers, sample_documents):
    ...
    @pytest.mark.asyncio
    async def test_process_documents_exception_in_pipeline(self, document_pipeline, mock_workers, sample_documents):
    ...
    @pytest.mark.asyncio
    async def test_process_documents_exception_in_embedding(self, document_pipeline, mock_workers, sample_documents):
    ...
    @pytest.mark.asyncio
    async def test_process_documents_exception_in_upsert(self, document_pipeline, mock_workers, sample_documents):
        ...
        chunking_worker.process_documents.return_value = chunks_iter
        embedding_worker.process_chunks.return_value = embedded_chunks_iter
        upsert_worker.process_embedded_chunks = AsyncMock(side_effect=Exception("Upsert failed"))
    ...
    async def test_logging_during_processing(self, mock_logger, document_pipeline, mock_workers, sample_documents):
        ...
        assert any("Processing 2 documents through pipeline" in msg for msg in log_messages)
        assert any("Starting chunking phase" in msg for msg in log_messages)
        assert any("Chunking completed, transitioning to embedding phase" in msg for msg in log_messages)
    ...
    @pytest.mark.asyncio
    @patch('qdrant_loader.core.pipeline.document_pipeline.logger')
    async def test_error_logging(self, mock_logger, document_pipeline, mock_workers, sample_documents):
    ...
    @pytest.mark.asyncio
    @patch('qdrant_loader.core.pipeline.document_pipeline.logger')
    async def test_timing_and_completion_logging(self, mock_logger, document_pipeline, mock_workers, sample_documents):
        ...
        assert any("Chunking phase took" in msg and "seconds" in msg for msg in log_messages)
        assert any("Embedding + Upsert phase took" in msg and "seconds" in msg for msg in log_messages)
        assert any("Total pipeline duration:" in msg and "seconds" in msg for msg in log_messages)
        
        # Check completion logging with specific counts
        assert any("5 chunks processed, 2 errors" in msg for msg in log_messages)

    @pytest.mark.asyncio
    async def test_process_documents_with_single_document(self, document_pipeline, mock_workers):
    ...
    @pytest.mark.asyncio
    async def test_worker_interaction_order(self, document_pipeline, mock_workers, sample_documents):
