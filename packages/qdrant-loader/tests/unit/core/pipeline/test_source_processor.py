...
class TestSourceProcessor:
    ...
    @pytest.mark.asyncio
    async def test_process_source_type_success(self, mock_source_config, sample_documents):
        ...
        with patch('qdrant_loader.core.pipeline.source_processor.logger') as mock_logger:
    ...
    async def test_process_source_type_multiple_sources(self, sample_documents):
        ...
        source_configs = {
            "source_1": source_config_1,
            "source_2": source_config_2
        }
        ...
        for i, docs in enumerate([sample_documents[:1], sample_documents[1:]]):
    ...
    @pytest.mark.asyncio
    async def test_process_source_type_with_file_conversion_config(self, file_conversion_config):
        ...
        mock_connector_instance.set_file_conversion_config.assert_called_once_with(file_conversion_config)
    ...
    @pytest.mark.asyncio
    async def test_process_source_type_file_conversion_disabled(self, file_conversion_config):
    ...
    @pytest.mark.asyncio
    async def test_process_source_type_connector_without_file_conversion(self, file_conversion_config):
    ...
    async def test_process_source_type_partial_failure(self, sample_documents):
        ...
        source_configs = {
            "good_source": source_config_1,
            "bad_source": source_config_2
        }
    ...
    async def test_process_source_type_logging_calls(self, mock_logger, mock_source_config, sample_documents):
        ...
        await processor.process_source_type(
            source_configs, mock_connector_class, "git"
        )
        ...
        assert any("Retrieved 2 documents from git source: test_source" in msg for msg in debug_calls)
