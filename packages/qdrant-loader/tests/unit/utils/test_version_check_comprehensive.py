...
class TestVersionChecker:
    ...
    @patch(
        ...
        read_data='{"timestamp": 1234567890, "latest_version": "2.0.0"}',
    )
    @patch("time.time")
    ...
    @patch(
        "qdrant_loader.utils.version_check.urlopen",
    ...
    @patch(
        "qdrant_loader.utils.version_check.urlopen",
        side_effect=HTTPError("http://test", 404, "Not Found", {}, None),
    )
    @patch("qdrant_loader.utils.version_check.Request")
    ...
    @patch.object(VersionChecker, "_get_cache_data")
    def test_check_for_updates_fetch_and_cache(
        self, mock_get_cache, mock_fetch, mock_save_cache
    ):
...
class TestCheckVersionAsync:
    ...
    def test_check_version_async_internal_function(
        self, mock_check_updates, mock_show_notification, mock_thread
    ):
    ...
    @patch.object(VersionChecker, "show_update_notification")
    @patch.object(VersionChecker, "check_for_updates")
    def test_check_version_async_no_update(
        self, mock_check_updates, mock_show_notification
    ):
    ...
    @patch.object(VersionChecker, "show_update_notification")
    @patch.object(VersionChecker, "check_for_updates")
    def test_check_version_async_silent_no_notification(
        self, mock_check_updates, mock_show_notification
    ):
