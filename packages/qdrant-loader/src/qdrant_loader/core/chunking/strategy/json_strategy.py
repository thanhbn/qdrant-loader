...
class JSONChunkingStrategy(BaseChunkingStrategy):
    ...
    def __init__(self, settings: Settings):
        ...
        self.simple_chunking_threshold = 500_000  # Use simple chunking for files larger than 500KB
    ...
    def chunk_document(self, document: Document) -> List[Document]:
        ...
        try:
            ...
            if not document_structure.get("valid_json", False):
                self.progress_tracker.log_fallback(document.id, "Invalid JSON structure")
                return self._fallback_chunking(document)
            ...
            final_elements = self.section_splitter.split_json_elements(elements_to_chunk)
            ...
            for i, element in enumerate(final_elements):
                ...
                element_metadata = self.metadata_extractor.extract_json_element_metadata(element)
                
                # Extract hierarchical metadata from content
                hierarchical_metadata = self.metadata_extractor.extract_hierarchical_metadata(
            ...
            # Log completion
            self.progress_tracker.finish_chunking(document.id, len(chunked_docs), "json")
            self.logger.info(
        ...
        try:
            ...
            for line in lines:
                ...
                if current_size + line_size > chunk_size and current_chunk_lines:
                    ...
                    overlap_lines = current_chunk_lines[-overlap//50:] if overlap > 0 else []
    ...
    def estimate_chunk_count(self, document: Document) -> int:
        ...
        try:
            ...
            if structure.get("valid_json", False):
                ...
                # Fallback estimation
                return max(1, len(document.content) // self.settings.global_config.chunking.chunk_size)
