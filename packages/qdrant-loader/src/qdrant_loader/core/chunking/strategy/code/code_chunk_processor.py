...
class CodeChunkProcessor(BaseChunkProcessor):
    ...
    def create_chunk_document(
        ...
        base_metadata = self.create_base_chunk_metadata(original_doc, chunk_index, total_chunks, chunk_metadata)
    ...
    def should_skip_semantic_analysis(self, chunk_content: str, chunk_metadata: dict[str, Any]) -> tuple[bool, str]:
    ...
    def _create_code_specific_metadata(self, content: str, chunk_metadata: dict[str, Any], original_doc: Document) -> dict[str, Any]:
        return {
            ...
            "element_context": self._extract_element_context(content, chunk_metadata.get("element_type", "unknown"))
            if chunk_metadata.get("element_type", "unknown") != "unknown"
            else None,
        }

    def _generate_chunk_title(self, original_doc: Document, chunk_metadata: dict[str, Any], chunk_index: int) -> str:
    ...
    def _is_generated_code(self, content: str) -> bool:
        lower = content.lower()
        return any(pat in lower for pat in self.skip_conditions["generated_code_patterns"])
    ...
    def _extract_element_context(self, content: str, element_type: str) -> dict[str, Any]:
        ...
        if element_type in ["function", "method"]:
            ...
            context["param_count_estimate"] = content.split("(", 1)[-1].split(")")[0].count(",") + 1 if "(" in content and ")" in content else 0
