...
def extract_language_context(content: str, chunk_metadata: dict[str, Any]) -> dict[str, Any]:
...
def _identify_frameworks(content: str, language: str) -> list[str]:
    lower = content.lower()
    if language in ["python"] and any(k in lower for k in ["django", "flask", "fastapi"]):
        return [k for k in ["django", "flask", "fastapi"] if k in lower]
    if language in ["javascript", "typescript"] and any(k in lower for k in ["react", "vue", "angular", "next"]):
...
def _identify_language_version(content: str, language: str) -> str:
    if language == "python" and (":=" in content or "match " in content):
        return "3.x"
    if language in ["javascript", "typescript"] and any(k in content for k in ["=>", "const", "let"]):
        return "ES6+"
    return "unknown"
