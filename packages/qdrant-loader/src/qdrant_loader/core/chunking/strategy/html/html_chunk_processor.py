...
class HTMLChunkProcessor(BaseChunkProcessor):
    ...
    def _generate_chunk_title(self, content: str, chunk_index: int, original_doc: Document) -> str:
        """Generate a descriptive title for the HTML chunk."""
        try:
            # Try to extract title from HTML content using metadata extractor
            section_title = self.metadata_extractor.document_parser.extract_section_title(content)
    ...
    def _determine_skip_reason(self, content: str, chunk_metadata: Dict[str, Any]) -> str:
