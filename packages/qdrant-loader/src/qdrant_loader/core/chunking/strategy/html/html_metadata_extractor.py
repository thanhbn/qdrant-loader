...
class HTMLMetadataExtractor(BaseMetadataExtractor):
    ...
    def extract_entities(self, text: str) -> List[str]:
        ...
        try:
            ...
            semantic_elements = soup.find_all(list(self.document_parser.section_elements))
    ...
    def _build_dom_path_breadcrumb(self, soup: BeautifulSoup) -> str:
        ...
        try:
            ...
            for element in soup.find_all():
                if (element.name in self.document_parser.section_elements or 
                    element.name in self.document_parser.heading_elements or
                    element.get('id') or 
                    element.get('role')):
    ...
    def _calculate_accessibility_score(self, soup: BeautifulSoup) -> float:
        ...
        try:
            ...
            semantic_count = len(soup.find_all(list(self.document_parser.section_elements)))
            if semantic_count > 0:
                score += min(semantic_count / 3.0, 1.0)  # Up to 1 point for semantic elements
    ...
    def _analyze_media_elements(self, soup: BeautifulSoup) -> Dict[str, Any]:
        ...
        try:
            ...
            images = soup.find_all("img")
            if images:
                media["images_with_alt"] = len([img for img in images if img.get("alt")])
                media["images_with_title"] = len([img for img in images if img.get("title")])
                media["responsive_images"] = len([img for img in images if img.get("srcset")])
    ...
    def _analyze_html_features(self, soup: BeautifulSoup) -> Dict[str, Any]:
        ...
        try:
            ...
            features["html5_semantic_tags"] = len(soup.find_all(list(self.document_parser.section_elements)))
            ...
            features["css_classes"] = sum(len(elem.get("class", [])) for elem in elements_with_class)
    ...
    def _assess_markup_quality(self, soup: BeautifulSoup) -> Dict[str, Any]:
        ...
        try:
            ...
            quality["deprecated_tags"] = sum(len(soup.find_all(tag)) for tag in deprecated_tags)
