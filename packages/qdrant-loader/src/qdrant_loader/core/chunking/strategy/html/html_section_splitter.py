...
class HTMLSectionSplitter(BaseSectionSplitter):
    ...
    def split_sections(self, content: str, document: Optional[Document] = None) -> List[Dict[str, Any]]:
        ...
        try:
            ...
            if len(content) <= self.simple_parsing_threshold and self.preserve_semantic_structure:
            ...
            def process_element(element, level=0, parent_path=""):
                ...
                if hasattr(element, "children") and level < self.max_recursion_depth:
                    for child in element.children:
                        process_element(child, level + 1, current_path if isinstance(element, Tag) else parent_path)
    ...
    def _merge_small_sections(self, sections: List[Dict[str, Any]]) -> List[Dict[str, Any]]:
        ...
        for section in sections:
            ...
            if (
                section_size >= min_section_size or
                section.get("tag_name") in self.document_parser.section_elements or
                section.get("tag_name") in self.document_parser.heading_elements
            ):
    ...
    def _split_large_sections(self, sections: List[Dict[str, Any]]) -> List[Dict[str, Any]]:
