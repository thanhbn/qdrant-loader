...
class TextSectionSplitter(BaseSectionSplitter):
        ...
        for i, paragraph in enumerate(paragraphs):
            ...
            sections.append({
                ...
                'metadata': {
                    ...
                    'content_characteristics': self._analyze_paragraph_content(paragraph)
                }
            })
    ...
    def _merge_small_sections(self, sections: List[Dict[str, Any]]) -> List[Dict[str, Any]]:
        ...
        for section in sections:
            ...
            if (len(content) >= self.min_chunk_size and not current_content) or \
               len(current_content + " " + content) > self.chunk_size:
    ...
    def _has_special_formatting(self, text: str) -> bool:
        ...
        return any(re.search(pattern, text, re.MULTILINE) for pattern in formatting_patterns)
    ...
    def _analyze_paragraph_content(self, paragraph: str) -> Dict[str, Any]:
        ...
        return {
            ...
            'avg_sentence_length': len(paragraph) / max(1, len(re.split(r'[.!?]+', paragraph))),
            'has_questions': '?' in paragraph,
            'has_exclamations': '!' in paragraph,
            'capitalization_ratio': len(re.findall(r'[A-Z]', paragraph)) / max(1, len(paragraph)),
