...
class TextChunkProcessor(BaseChunkProcessor):
    ...
    def _create_text_specific_metadata(self, content: str, original_doc: Document) -> Dict[str, Any]:
    ...
    def _calculate_quality_metrics(self, content: str) -> Dict[str, Any]:
        ...
        return {
            ...
            "readability_score": self._estimate_readability(avg_sentence_length, word_diversity),
    ...
    def _extract_semantic_indicators(self, content: str) -> Dict[str, Any]:
        ...
        business_keywords = ['company', 'business', 'market', 'revenue', 'profit', 'customer']
        tech_keywords = ['software', 'technology', 'system', 'data', 'algorithm', 'code']
        academic_keywords = ['research', 'study', 'analysis', 'theory', 'methodology', 'conclusion']
        ...
        return {
            ...
            "discourse_markers": {
                "enumeration": bool(re.search(r'\b(first|second|third|finally|lastly)\b', content_lower)),
                "causation": bool(re.search(r'\b(because|therefore|thus|consequently|as a result)\b', content_lower)),
                "contrast": bool(re.search(r'\b(however|although|despite|nevertheless|on the other hand)\b', content_lower)),
                "comparison": bool(re.search(r'\b(similarly|likewise|compared to|in contrast)\b', content_lower)),
            },
            "complexity_indicators": {
                "has_subordinate_clauses": bool(re.search(r'\b(which|that|who|whom|whose)\b', content_lower)),
                "has_conditionals": bool(re.search(r'\b(if|unless|provided|assuming)\b', content_lower)),
                "has_temporal_references": bool(re.search(r'\b(when|while|before|after|during)\b', content_lower)),
            }
        }
    ...
    def _extract_entity_hints(self, content: str) -> Dict[str, Any]:
        ...
        return {
            ...
            "potential_names": len([noun for noun in proper_nouns if len(noun.split()) <= 3]),
            "potential_organizations": len([noun for noun in proper_nouns if len(noun.split()) > 1]),
            "has_titles": bool(re.search(r'\b(Dr|Mr|Mrs|Ms|Prof|CEO|CTO|VP)\b\.?\s+[A-Z]', content)),
            "has_locations": bool(re.search(r'\b[A-Z][a-z]+(?:\s+[A-Z][a-z]+)*(?:\s+(?:City|State|Country|Street|Ave|Road))\b', content)),
        }

    def _estimate_readability(self, avg_sentence_length: float, word_diversity: float) -> str:
