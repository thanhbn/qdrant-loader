...
class JSONDocumentParser(BaseDocumentParser):
    ...
    def parse_document_structure(self, content: str) -> Dict[str, Any]:
        ...
        try:
            ...
            structure = {
                ...
                "array_stats": self._analyze_arrays(data) if isinstance(data, (list, dict)) else {}
            }
    ...
    def parse_json_structure(self, content: str) -> Optional[JSONElement]:
        ...
        try:
            ...
            # Create root element
            root_type = JSONElementType.OBJECT if isinstance(data, dict) else (
                JSONElementType.ARRAY if isinstance(data, list) else JSONElementType.VALUE
            )
            
            root_element = self._create_json_element(
                "root", data, root_type, "$", 0
            )
    ...
    def _infer_basic_schema(self, data: Any) -> Dict[str, Any]:
        ...
        if isinstance(data, dict):
            return {
                "type": "object",
                "properties": {key: self._infer_basic_schema(value) for key, value in data.items()},
