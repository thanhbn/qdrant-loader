...
class JSONMetadataExtractor(BaseMetadataExtractor):
    ...
    def extract_hierarchical_metadata(self, content: str, chunk_metadata: Dict[str, Any], document: Document) -> Dict[str, Any]:
        ...
        try:
            ...
            metadata.update({
                ...
                "key_patterns": self._analyze_key_patterns(data) if isinstance(data, dict) else [],
            ...
            metadata.update({
                ...
                "configuration_indicators": self._detect_configuration_patterns(data)
            })
    ...
    def extract_json_element_metadata(self, element: JSONElement) -> Dict[str, Any]:
        ...
        if isinstance(element.value, dict):
            metadata["data_types"] = list({type(v).__name__ for v in element.value.values()})
            metadata["has_nested_objects"] = any(isinstance(v, dict) for v in element.value.values())
            metadata["has_arrays"] = any(isinstance(v, list) for v in element.value.values())
            ...
            metadata["has_nested_objects"] = any(isinstance(v, dict) for v in element.value)
            metadata["has_arrays"] = any(isinstance(v, list) for v in element.value)
            metadata["array_length"] = len(element.value)
            metadata["array_homogeneity"] = self._analyze_array_homogeneity(element.value)
        ...
        if isinstance(data, dict):
            ...
            if data:
                ...
                for item in data[:sample_size]:
                    schema["item_schemas"].append(self._infer_comprehensive_schema(item))
    ...
    def _calculate_complexity_score(self, data: Any) -> float:
        ...
        if isinstance(data, dict):
            return 1.0 + sum(self._calculate_complexity_score(value) for value in data.values()) * 0.5
        elif isinstance(data, list):
            return 1.0 + sum(self._calculate_complexity_score(item) for item in data) * 0.3
        else:
            return 0.1
        ...
        def analyze_value(value, path="$"):
            if value is None:
                ...
                if "min" not in distributions["numeric_ranges"]:
                    ...
                    distributions["numeric_ranges"]["min"] = min(distributions["numeric_ranges"]["min"], value)
                    distributions["numeric_ranges"]["max"] = max(distributions["numeric_ranges"]["max"], value)
                ...
                if "min_length" not in distributions["string_length_stats"]:
                    ...
                    distributions["string_length_stats"]["min_length"] = min(distributions["string_length_stats"]["min_length"], length)
                    distributions["string_length_stats"]["max_length"] = max(distributions["string_length_stats"]["max_length"], length)
            elif isinstance(value, bool):
                distributions["boolean_distribution"][str(value)] = distributions["boolean_distribution"].get(str(value), 0) + 1
    ...
    def _analyze_array_statistics(self, data: Any) -> Dict[str, Any]:
        ...
        def find_arrays(obj, path="$"):
            if isinstance(obj, list):
                array_info = {
                    "path": path,
                    "length": len(obj),
                    "item_types": [type(item).__name__ for item in obj[:5]],  # Sample first 5
                    "homogeneous": len(set(type(item).__name__ for item in obj)) == 1 if obj else True,
        ...
        return {
            ...
            "nested_structure_complexity": sum(1 for arr in arrays_found if arr["nested_arrays"] or arr["nested_objects"])
        }
        ...
        def check_uniqueness(obj, path="$"):
            if isinstance(obj, str):
                if obj in uniqueness["unique_strings"]:
                    uniqueness["repeated_values"][obj] = uniqueness["repeated_values"].get(obj, 1) + 1
                else:
                    uniqueness["unique_strings"].add(obj)
                    
                # Check if looks like an ID
                if (len(obj) >= 8 and 
                    (obj.isalnum() or '-' in obj or '_' in obj) and 
                    any(c.isdigit() for c in obj)):
    ...
    def _detect_relationships(self, data: Any) -> List[str]:
        ...
        if isinstance(data, dict):
            ...
            if any(isinstance(value, list) and value and isinstance(value[0], dict) for value in data.values()):
    ...
    def _estimate_object_completeness(self, obj: dict) -> float:
        ...
        non_null_fields = sum(1 for value in obj.values() if value is not None and value != "")
    ...
    def _analyze_simple_value(self, value: Any) -> Dict[str, Any]:
        ...
        if isinstance(value, str):
            analysis.update({
                "length": len(value),
                "format_hints": self._detect_value_format(value)
            })
        elif isinstance(value, (int, float)):
            analysis.update({
                "numeric_value": value,
    ...
    def _check_uniform_structure(self, objects: list) -> bool:
        ...
        return all(set(obj.keys()) == first_keys for obj in objects[1:5])  # Check first 5
    ...
    def _extract_json_entities(self, data: Any, path: str = "$") -> List[str]:
        ...
        if isinstance(data, dict):
            for key, value in data.items():
                ...
                if isinstance(value, str) and len(value) > 2 and not self._is_url(value):
    ...
    def _extract_text_entities(self, text: str) -> List[str]:
        ...
        entities.extend([s for s in quoted_strings if s.replace("_", "").replace("-", "").isalpha()])
