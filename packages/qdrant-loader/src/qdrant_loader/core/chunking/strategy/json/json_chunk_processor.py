...
class JSONChunkProcessor(BaseChunkProcessor):
    ...
    def create_chunk_document(
        ...
        should_skip_nlp = skip_nlp or self._should_skip_nlp_for_json(chunk_content, chunk_metadata)
        ...
        chunk_doc = Document(
            ...
            metadata=self._create_enhanced_metadata(original_doc, chunk_metadata, chunk_index, total_chunks)
        )
    ...
    def _should_skip_nlp_for_json(self, content: str, metadata: Dict[str, Any]) -> bool:
        ...
        if json_type in ["list", "dict"] and metadata.get("structure_type") in [
            "primitive_collection", "configuration", "data_container"
        ]:
            return True
    ...
    def _is_minimal_text_content(self, content: str) -> bool:
        ...
        try:
            ...
            text_chars = 0
            structural_chars = content.count('{') + content.count('}') + content.count('[') + content.count(']') + content.count(',') + content.count(':')
    ...
    def _is_configuration_structure(self, metadata: Dict[str, Any]) -> bool:
        ...
        config_schema_patterns = ["configuration_object", "feature_flags", "typed_value"]
    ...
    def _create_enhanced_metadata(
        ...
        enhanced_metadata.update({
            "chunk_index": chunk_index,
            "total_chunks": total_chunks,
            "chunk_size": chunk_metadata.get("chunk_size", len(chunk_metadata.get("content", ""))),
        ...
        enhanced_metadata.update({
            ...
            "chunk_quality_indicators": self._calculate_chunk_quality_indicators(chunk_metadata)
        })
        
        return enhanced_metadata
    
    def _calculate_chunk_quality_indicators(self, chunk_metadata: Dict[str, Any]) -> Dict[str, Any]:
        ...
        # Schema coherence
        if chunk_metadata.get("is_valid_json", True) == False:
