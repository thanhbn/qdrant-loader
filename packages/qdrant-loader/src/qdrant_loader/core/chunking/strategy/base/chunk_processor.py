...
class BaseChunkProcessor(ABC):
    ...
    def create_chunk_document(self, original_doc: "Document", chunk_content: str, 
        ...
        """
        raise NotImplementedError("Chunk processor must implement create_chunk_document method")
    ...
    def estimate_chunk_count(self, content: str) -> int:
        ...
        effective_chunk_size = max(1, self.chunk_size - self.settings.global_config.chunking.chunk_overlap)
        estimated = max(1, (content_size + effective_chunk_size - 1) // effective_chunk_size)
