...
def parse_attachment(raw_attachment: Dict[str, Any]) -> JiraAttachment:
    ...
    try:
        ...
        raise ValueError(f"Invalid created timestamp in attachment: {created_raw!r}") from e
...
def parse_issue(raw_issue: Dict[str, Any]) -> JiraIssue:
    ...
    if not isinstance(fields, dict):
        raise ValueError(f"Jira issue {issue_identifier} missing required 'fields' object")

    # Validate required top-level keys within fields
    required_field_keys = ["summary", "created", "updated", "reporter"]
    missing_simple = [k for k in required_field_keys if k not in fields or fields.get(k) is None]
    if missing_simple:
        raise ValueError(
    ...
    def _require_dict_with_key(container: Dict[str, Any], outer_key: str, inner_key: str) -> None:
        value = container.get(outer_key)
        if not isinstance(value, dict) or inner_key not in value or value.get(inner_key) is None:
            raise ValueError(
    ...
    try:
        created_dt = (
            datetime.fromisoformat(created_raw.replace("Z", "+00:00")) if isinstance(created_raw, str) else None
        )
    except Exception as e:
        raise ValueError(
    ...
    if created_dt is None:
        raise ValueError(f"Jira issue {issue_identifier} missing valid 'created' timestamp")

    try:
        updated_dt = (
            datetime.fromisoformat(updated_raw.replace("Z", "+00:00")) if isinstance(updated_raw, str) else None
        )
    except Exception as e:
        raise ValueError(
    ...
    if updated_dt is None:
        raise ValueError(f"Jira issue {issue_identifier} missing valid 'updated' timestamp")
    ...
    subtasks_keys = [st.get("key") for st in raw_subtasks if isinstance(st, dict) and st.get("key")]
    ...
    linked_outward = [
        ...
        if isinstance(link, dict) and isinstance(link.get("outwardIssue"), dict) and link.get("outwardIssue", {}).get("key")
    ]
    ...
    return JiraIssue(
        ...
        attachments=[parse_attachment(att) for att in attachments_list if isinstance(att, dict)],
        comments=[parse_comment(comment) for comment in comments_list if isinstance(comment, dict)],
