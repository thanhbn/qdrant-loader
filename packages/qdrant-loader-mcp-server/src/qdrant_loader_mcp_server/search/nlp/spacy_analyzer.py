...
class SpaCyQueryAnalyzer:
    ...
    def __init__(self, spacy_model: str = "en_core_web_md"):
        ...
        self.intent_patterns = {
            "technical_lookup": {
                ...
                "keywords": {"api", "database", "architecture", "implementation", "system", "code", "function"},
                "question_words": set(),
            },
            "business_context": {
                "entities": {"ORG", "MONEY", "PERCENT", "CARDINAL"},
                "pos_sequences": [["NOUN", "NOUN"], ["ADJ", "NOUN", "NOUN"]],
                "keywords": {"requirements", "objectives", "strategy", "business", "scope", "goals"},
                "question_words": {"what", "why", "how"},
            },
            "vendor_evaluation": {
                "entities": {"ORG", "MONEY", "PERSON"},
                "pos_sequences": [["NOUN", "NOUN"], ["VERB", "NOUN"], ["ADJ", "NOUN"]],
                "keywords": {"proposal", "criteria", "cost", "vendor", "comparison", "evaluation"},
                "question_words": {"which", "what", "how much"},
            },
            "procedural": {
                "entities": set(),
                "pos_sequences": [["VERB", "NOUN"], ["VERB", "DET", "NOUN"]],
                "keywords": {"how", "steps", "process", "procedure", "guide", "tutorial"},
    ...
    def analyze_query_semantic(self, query: str) -> QueryAnalysis:
        ...
        main_concepts = [chunk.text.strip() for chunk in doc.noun_chunks if len(chunk.text.strip()) > 2]
        
        # Detect intent using linguistic patterns
        intent_signals = self._detect_intent_patterns(doc, entities, pos_patterns, semantic_keywords)
    ...
    def semantic_similarity_matching(self, query_analysis: QueryAnalysis, entity_text: str) -> float:
        ...
        try:
            ...
            if query_analysis.query_vector.has_vector and entity_doc.has_vector:
                ...
                similarity = self._token_similarity_fallback(
                    query_analysis.semantic_keywords, 
                    entity_text.lower()
                )
    ...
    def _detect_intent_patterns(
        ...
        for intent_name, pattern in self.intent_patterns.items():
            ...
            entity_match = len(entity_labels.intersection(pattern["entities"])) / max(len(pattern["entities"]), 1)
            score += entity_match * 0.3
            
            # POS sequence matching
            pos_match = self._match_pos_sequences(pos_patterns, pattern["pos_sequences"])
            score += pos_match * 0.3
            
            # Keyword matching
            keyword_match = len(keyword_set.intersection(pattern["keywords"])) / max(len(pattern["keywords"]), 1)
        ...
        return {
            ...
            "linguistic_features": {
                ...
                "has_question_words": any(token.text.lower() in {"what", "how", "why", "when", "who", "where"} for token in doc),
                "verb_count": sum(1 for pos in pos_patterns if pos in {"VERB", "AUX"}),
                "noun_count": sum(1 for pos in pos_patterns if pos == "NOUN"),
            }
        }
    
    def _match_pos_sequences(self, pos_patterns: List[str], target_sequences: List[List[str]]) -> float:
    ...
    def _match_question_words(self, doc: Doc, question_words: Set[str]) -> float:
        ...
        found_words = {token.text.lower() for token in doc if token.text.lower() in question_words}
    ...
    def _is_question(self, doc: Doc) -> bool:
        ...
        question_words = {"what", "how", "why", "when", "who", "where", "which", "whose", "whom"}
        ...
        if len(doc) >= 2:
            ...
            aux_patterns = {("can", "you"), ("do", "we"), ("is", "there"), ("are", "there"), ("will", "you")}
            if tuple(first_two) in aux_patterns:
                return True
        
        return False
    
    def _is_technical_query(self, doc: Doc, entities: List[Tuple[str, str]], keywords: List[str]) -> bool:
        """Detect if the query is technical in nature."""
        technical_indicators = {
            "api", "database", "system", "code", "function", "architecture", 
            "implementation", "framework", "library", "server", "client",
            "protocol", "algorithm", "data", "query", "schema", "endpoint"
        }
        ...
        technical_entities = {"ORG", "PRODUCT", "LANGUAGE"}  # Often technical in this context
        entity_labels = {label for _, label in entities}
    ...
    def _calculate_complexity_score(self, doc: Doc) -> float:
        ...
        factors = {
            ...
            "noun_chunks": min(len(list(doc.noun_chunks)) / 5, 1.0),  # More concepts = more complex
            "question_words": min(sum(1 for token in doc if token.text.lower() in 
                                    {"what", "how", "why", "when", "who", "where", "which"}) / 3, 1.0),
            "dependency_depth": min(self._max_dependency_depth(doc) / 5, 1.0),
        }
        
        # Weighted average
        weights = {"length": 0.2, "entities": 0.3, "noun_chunks": 0.2, "question_words": 0.15, "dependency_depth": 0.15}
    ...
    def _token_similarity_fallback(self, query_keywords: List[str], entity_text: str) -> float:
