...
class EntityQueryExpander:
    ...
    def expand_query(
        ...
        try:
            ...
            entity_terms = self._expand_with_entities(
                query_analysis, search_context
            )
            expansion_terms.extend(entity_terms)
            
            # 3. Concept-based expansion using noun chunks
            concept_terms = self._expand_with_concepts(
                query_analysis, search_context
            )
    ...
    def _get_entity_synonyms(self, entity_text: str, entity_type: str) -> List[str]:
        ...
        type_synonyms = {
            "ORG": lambda text: [text.lower(), f"{text.lower()} company", f"{text.lower()} organization"],
            "PRODUCT": lambda text: [text.lower(), f"{text.lower()} software", f"{text.lower()} tool"],
            "PERSON": lambda text: [text.lower(), f"{text.lower()} developer", f"{text.lower()} author"],
    ...
    def _filter_expansion_terms(
        ...
        for term in expansion_terms:
            ...
            if (term_clean not in original_set and 
