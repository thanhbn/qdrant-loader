...
class GraphTraverser:
    ...
    def _calculate_semantic_score(
        ...
        total_score = (
            entity_similarity * 0.4 + topic_similarity * 0.3 + keyword_similarity * 0.3
        )

        return total_score

    def _calculate_list_similarity(
        self, list1: list[tuple[str, str]], list2: list[tuple[str, str]]
    ) -> float:
    ...
    def _build_reasoning_path(
        self, path: list[str], edges: list[GraphEdge]
    ) -> list[str]:
...
class GraphBuilder:
    ...
    def _create_relationships(
        self, search_results: list[SearchResult], graph: KnowledgeGraph
    ) -> list[GraphEdge]:
    ...
    def _calculate_node_similarity(self, node1: GraphNode, node2: GraphNode) -> float:
        ...
        entity_similarity = self._jaccard_similarity(
            set(node1.entities), set(node2.entities)
        )

        # Topic overlap
        topic_similarity = self._jaccard_similarity(
            set(node1.topics), set(node2.topics)
        )

        # Keyword overlap
        keyword_similarity = self._jaccard_similarity(
            set(node1.keywords), set(node2.keywords)
        )
    ...
    def _extract_topics(self, result: SearchResult) -> list[str]:
        ...
        if result.breadcrumb_text:
            topics.extend(
                [section.strip() for section in result.breadcrumb_text.split(" > ")]
            )
    ...
    def _extract_keywords(self, result: SearchResult) -> list[str]:
        ...
        keywords.extend(
            [word for word in text_words if len(word) > 3 and word.isalpha()]
        )
