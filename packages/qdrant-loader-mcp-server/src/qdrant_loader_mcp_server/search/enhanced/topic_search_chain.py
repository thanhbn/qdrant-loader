...
class ChainStrategy(Enum):
    ...
    RELEVANCE_RANKED = "relevance_ranked" # Order by semantic relevance to original query
...
class TopicChainLink:
    ...
    # Chain context
    parent_query: Optional[str] = None   # Query that led to this one
...
class TopicSearchChain:
    ...
    spacy_analysis: Optional[QueryAnalysis] = None
...
class TopicRelationshipMap:
    ...
    def __init__(self, spacy_analyzer: SpaCyQueryAnalyzer):
        ...
        self.topic_cooccurrence: Dict[str, Dict[str, int]] = defaultdict(lambda: defaultdict(int))
    ...
    def build_topic_map(self, search_results: List[SearchResult]) -> None:
        ...
        logger.info(f"Building topic relationship map from {len(search_results)} search results")
        ...
        logger.info(
            ...
            total_cooccurrences=sum(len(cooc) for cooc in self.topic_cooccurrence.values())
        )
    ...
    def find_related_topics(
        ...
        if include_semantic:
            ...
            semantic_related = self._find_semantic_related_topics(source_topic, max_related)
        ...
        if include_cooccurrence:
            ...
            cooccurrence_related = self._find_cooccurrence_related_topics(source_topic, max_related)
    ...
    def _find_semantic_related_topics(self, source_topic: str, max_related: int) -> List[Tuple[str, float]]:
        ...
        for topic in self.topic_document_frequency.keys():
            ...
            if similarity > self.similarity_threshold:
                ...
                doc_freq_weight = min(1.2, 1.0 + (self.topic_document_frequency[topic] / 100))
    ...
    def _find_cooccurrence_related_topics(self, source_topic: str, max_related: int) -> List[Tuple[str, float]]:
        ...
        for topic, cooccur_count in self.topic_cooccurrence[source_topic].items():
            if cooccur_count >= self.cooccurrence_threshold:
                ...
                pmi = math.log2((cooccur_count * total_docs) / (source_freq * topic_freq + 1))
...
class TopicSearchChainGenerator:
    ...
    def generate_search_chain(
        ...
        discovery_potential = self._calculate_discovery_potential(chain_links, spacy_analysis)
    ...
    def _extract_primary_topics(self, spacy_analysis: QueryAnalysis, query: str) -> List[str]:
        ...
        for entity_text, entity_label in spacy_analysis.entities:
            ...
            if link_idx == 0:
                ...
                if primary_topics:
                    ...
                    if related_topics:
                        ...
                        related_topic_names = [topic for topic, score, rel_type in related_topics[:2]]
                        query = f"{primary_topic} related to {' and '.join(related_topic_names)}"
                        
                        chain_links.append(TopicChainLink(
                            query=query,
                            ...
                            entities=[ent[0] for ent in spacy_analysis.entities[:2]]
                        ))
                ...
                for explored_topic in list(explored_topics):
                    related = self.topic_map.find_related_topics(explored_topic, max_related=2)
                ...
                if candidate_topics:
                    ...
                    chain_links.append(TopicChainLink(
                        ...
                        parent_query=chain_links[-1].query if chain_links else original_query,
                        exploration_type="broader",
    ...
    def _generate_relevance_ranked_chain(
        ...
        for primary_topic in primary_topics:
            related = self.topic_map.find_related_topics(
                primary_topic, max_related=10, include_semantic=True, include_cooccurrence=True
            )
            for topic, score, rel_type in related:
                ...
                all_related_topics.append((topic, combined_score, rel_type, primary_topic))
        ...
        for link_idx in range(min(max_links, len(all_related_topics))):
            ...
            chain_links.append(TopicChainLink(
                ...
                parent_query=original_query if link_idx == 0 else chain_links[-1].query,
        ...
        for link_idx in range(max_links):
            if link_idx == 0:
                ...
                if relevance_links:
                    ...
                    relevance_link.parent_query = chain_links[-1].query if chain_links else original_query
    ...
    def _calculate_discovery_potential(
        ...
        # 1. Topic diversity
        unique_topics = set(link.topic_focus for link in chain_links)
        topic_diversity = len(unique_topics) / len(chain_links) if chain_links else 0
        
        # 2. Average relevance score
        avg_relevance = sum(link.relevance_score for link in chain_links) / len(chain_links)
        
        # 3. Exploration type diversity
        exploration_types = set(link.exploration_type for link in chain_links)
