...
class ConflictDetector:
    ...
    async def detect_conflicts(self, documents: list[SearchResult]) -> ConflictAnalysis:
        ...
        try:
            ...
            def analyze_pair(doc1: SearchResult, doc2: SearchResult, doc1_id: str, doc2_id: str) -> ConflictAnalysis | None:
                vector_similarity = 0.0
                if doc1_id in embeddings and doc2_id in embeddings:
                    vector_similarity = self._calculate_vector_similarity(embeddings[doc1_id], embeddings[doc2_id])
                    if vector_similarity > self.MAX_VECTOR_SIMILARITY or vector_similarity < self.MIN_VECTOR_SIMILARITY:
                        return None
                ...
                for j, doc2 in enumerate(documents[i + 1 :], i + 1):
                    ...
                    if baseline is None:
                        ...
                        if self.llm_enabled and vector_similarity > 0.7:
                            llm_conflict, llm_explanation, llm_confidence = await self._validate_conflict_with_llm(doc1, doc2, vector_similarity)
                            if llm_conflict:
                                conflicts.append(
                                    ConflictAnalysis(
                                        document1_title=doc1.source_title,
                    ...
                    if self.llm_enabled:
                        ...
                        if vector_similarity <= 0.0 and doc1_id in embeddings and doc2_id in embeddings:
                            vector_similarity = self._calculate_vector_similarity(embeddings[doc1_id], embeddings[doc2_id])
                        if vector_similarity > 0.7:
                            llm_conflict, llm_explanation, llm_confidence = await self._validate_conflict_with_llm(doc1, doc2, vector_similarity)
                            if llm_conflict:
                                baseline.confidence_score = max(baseline.confidence_score, llm_confidence)
                                baseline.explanation = baseline.explanation + f"; LLM: {llm_explanation}"
    ...
    def _extract_context_snippet(self, text, keyword, context_length=100, max_length=None):
        return _extract_context_snippet_ext(self, text, keyword, context_length, max_length)
