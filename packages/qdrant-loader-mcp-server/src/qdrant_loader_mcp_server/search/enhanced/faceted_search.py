...
class Facet:
    ...
    def get_top_values(self, limit: Optional[int] = None) -> List[FacetValue]:
...
class FacetedSearchResults:
    ...
    def get_facet(self, facet_type: FacetType) -> Optional[Facet]:
...
class DynamicFacetGenerator:
    ...
    def _generate_facet(
        ...
        facet_values = sorted(facet_values, key=lambda v: v.count, reverse=True)[:max_values]
    ...
    def _get_value_description(self, facet_type: FacetType, value: str) -> Optional[str]:
    ...
    def suggest_refinements(
        ...
        for facet in facets:
            ...
            for facet_value in facet.get_top_values(3):
                ...
                filtered_count = len(self.apply_facet_filters(current_results, [test_filter]))
                ...
                if 0 < filtered_count < len(current_results) * 0.8:  # 20%+ reduction
                    suggestions.append({
                        ...
                        "reduction_percent": round((1 - filtered_count / len(current_results)) * 100)
                    })
