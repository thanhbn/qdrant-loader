...
class TestComplementaryContentE2E:
    ...
    def test_complementary_content_finder_direct(
        self, real_complementary_finder, sample_documents
    ):
        ...
        for i, rec in enumerate(recommendations, 1):
            print(
                f"{i}. {rec['document_id']} - Score: {rec['relevance_score']:.3f} - {rec['recommendation_reason']}"
            )
    ...
    def test_scoring_algorithm_individual_cases(
        self, real_complementary_finder, sample_documents
    ):
        ...
        test_cases = [
            (
                sample_documents[1],
                "Business Requirements - should score for different content types",
            ),
            (
                sample_documents[2],
                "API Documentation - should score for different source types, shared entities",
            ),
            (
                sample_documents[3],
                "Security Guide - should score for cross-project insights",
            ),
            (
                sample_documents[4],
                "User Stories - should score for business vs technical complement",
            ),
        ]
        ...
        for candidate_doc, description in test_cases:
            score, reason = real_complementary_finder._calculate_complementary_score(
    ...
    def search_engine_config(
        self, mock_qdrant_client, mock_openai_client, mock_spacy_analyzer
    ):
    ...
    @pytest.mark.asyncio
    async def test_search_engine_complementary_content_integration(
        self, search_engine_config, sample_documents
    ):
        ...
        def create_mock_scroll_response(**kwargs):
            ...
            for i, doc in enumerate(
                sample_documents[:10]
            ):  # Limit to 10 docs for BM25 corpus
                content = doc.text if hasattr(doc, "text") else str(doc)
                mock_points.append(
                    MagicMock(
                        id=f"corpus_doc_{i}",
        ...
        with (
            patch(
                "qdrant_client.QdrantClient", return_value=config["mock_qdrant_client"]
            ),
            patch("openai.AsyncOpenAI", return_value=config["mock_openai_client"]),
            patch(
                "qdrant_loader_mcp_server.search.nlp.spacy_analyzer.SpaCyQueryAnalyzer",
                return_value=config["mock_spacy_analyzer"],
            ),
        ):

            await search_engine.initialize(
                config=config["qdrant_config"], openai_config=config["openai_config"]
            )
            ...
            async def mock_keyword_search(query, limit, project_ids=None):
                ...
                return (
                    []
                )  # Return empty results, focusing on vector search + complementary content
    ...
    async def test_mcp_handler_complementary_content_e2e(self, search_engine_config):
        ...
        try:
            ...
            if response and "result" in response:
                ...
                if content:
                    print(
                        f"Response content preview: {content[0].get('text', '')[:200]}..."
                    )
    ...
    def test_helper_methods_validation(
        self, real_complementary_finder, sample_documents
    ):
        ...
        has_shared_topics = real_complementary_finder._has_shared_topics(
            target_doc, api_doc
        )
        shared_topics_count = real_complementary_finder._get_shared_topics_count(
            target_doc, api_doc
        )
        print(
            f"Shared topics (tech + api): {has_shared_topics}, count: {shared_topics_count}"
        )

        # Test shared entities detection
        has_shared_entities = real_complementary_finder._has_shared_entities(
            target_doc, business_doc
        )
        shared_entities_count = real_complementary_finder._get_shared_entities_count(
            target_doc, business_doc
        )
        print(
            f"Shared entities (tech + business): {has_shared_entities}, count: {shared_entities_count}"
        )

        # Test new algorithm features
        different_doc_types = real_complementary_finder._has_different_document_types(
            target_doc, business_doc
        )
        print(f"Different document types (tech + business): {different_doc_types}")

        # Test abstraction levels
        abstraction_gap = real_complementary_finder._calculate_abstraction_gap(
            target_doc, business_doc
        )
        print(f"Abstraction gap (tech + business): {abstraction_gap}")

        # Test cross-functional relationships
        cross_functional = real_complementary_finder._has_cross_functional_relationship(
            target_doc, business_doc
        )
    ...
    @pytest.mark.asyncio
    async def test_improved_complementary_content_functionality(
        self, search_engine_config, sample_documents
    ):
    ...
    async def test_lightweight_response_structure(self, search_engine_config):
        ...
        with patch.object(
            search_engine, "find_complementary_content", return_value=mock_result
        ):
            request = {
                "jsonrpc": "2.0",
                "id": 1,
            ...
            print(
                f"Summary consistency: analyzed={summary['total_analyzed']}, found={summary['complementary_found']}"
            )

    @pytest.mark.asyncio
    async def test_adaptive_context_retrieval(
        self, search_engine_config, sample_documents
    ):
        ...
        with patch.object(search_engine.hybrid_search, "search") as mock_search:
            ...
            await search_engine.find_complementary_content(
                target_query="test", context_query="test", max_recommendations=2
            )

            # Verify the context search was called with adaptive limit
            context_call = mock_search.call_args_list[
                1
            ]  # Second call is context search
            ...
            await search_engine.find_complementary_content(
                target_query="test", context_query="test", max_recommendations=10
            )
            ...
            print("For max_recommendations=2: context_limit=20")
            print("For max_recommendations=10: context_limit=40")

    def test_fallback_scoring_mechanism(
        self, real_complementary_finder, sample_documents
    ):
        ...
        fallback_score = real_complementary_finder._calculate_fallback_score(
            target_doc, candidate_doc
        )
        ...
        fallback_score_minimal = real_complementary_finder._calculate_fallback_score(
            target_doc, minimal_doc
        )
...
if __name__ == "__main__":
    ...
    pytest.main(
        [
            __file__
            + "::TestComplementaryContentE2E::test_complementary_content_finder_direct",
            "-v",
            "-s",
        ]
    )
