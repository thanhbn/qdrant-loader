...
class TestProtocolComplianceAcrossTransports:
    ...
    @pytest.mark.asyncio
    async def test_tool_definitions_identical_across_transports(
        self, mcp_handler, test_client
    ):
        ...
        http_response = test_client.post(
            "/mcp", json=stdio_request, headers={"Origin": "http://localhost"}
        )
...
class TestStructuredOutputAcrossTransports:
    ...
    @pytest.mark.asyncio
    async def test_search_structured_output_stdio(
        self, mcp_handler, mock_query_processor, mock_search_engine
    ):
    ...
    def test_search_structured_output_http(
        self, test_client, mock_query_processor, mock_search_engine
    ):
        ...
        response = test_client.post(
            "/mcp", json=mcp_request, headers={"Origin": "http://localhost"}
        )
    ...
    async def test_structured_output_consistency(
        self, mcp_handler, test_client, mock_search_engine, mock_query_processor
    ):
...
class TestErrorHandlingAcrossTransports:
    ...
    def test_invalid_request_http(self, test_client):
        ...
        response = test_client.post(
            "/mcp", json=invalid_request, headers={"Origin": "http://localhost"}
        )
...
class TestTransportSpecificPerformance:
    ...
    @pytest.mark.asyncio
    async def test_stdio_direct_call_performance(
        self, mcp_handler, mock_search_engine, mock_query_processor
    ):
    ...
    def test_http_transport_multiple_requests(
        self, test_client, mock_search_engine, mock_query_processor
    ):
...
class TestEndToEndWorkflow:
    ...
    @pytest.mark.asyncio
    async def test_complete_stdio_workflow(
        self, mcp_handler, mock_search_engine, mock_query_processor
    ):
    ...
    def test_complete_http_workflow(
        self, test_client, mock_search_engine, mock_query_processor
    ):
