...
class TestCrossDocumentIntelligenceIntegration:
    ...
    async def test_end_to_end_document_analysis_workflow(self, intelligence_engine):
        ...
        expected_components = ["summary", "document_clusters", "relationships_count"]
    ...
    async def test_authentication_document_clustering(self, intelligence_engine):
        ...
        similarity_calculator = DocumentSimilarityCalculator(
            intelligence_engine.spacy_analyzer
        )
    ...
    async def test_complementary_content_discovery(self, intelligence_engine):
        ...
        [doc for doc in docs if "tutorial" in doc.source_title.lower()]
    ...
    async def test_citation_network_analysis(self, intelligence_engine):
        ...
        most_connected = citation_analyzer.get_most_connected_documents(
            network, limit=3
        )
        ...
        for doc_id, score in authoritative:
            ...
            assert isinstance(
                score, int | float
            )  # Authority scores can be negative in some graph configurations
    ...
    async def test_cross_project_analysis(self, intelligence_engine):
        ...
        clusters = cluster_analyzer.create_clusters(
            docs, strategy=ClusteringStrategy.PROJECT_BASED
        )
    ...
    async def test_document_relationship_discovery(self, intelligence_engine):
        ...
        oauth_doc = next(
            (doc for doc in docs if "oauth" in doc.source_title.lower()), None
        )
    ...
    async def test_performance_with_realistic_dataset(self, intelligence_engine):
        ...
        ), f"Processing dataset took too long: {processing_time:.2f}s"
    ...
    async def test_similarity_calculation_accuracy(self, intelligence_engine):
        ...
        oauth_jwt_similarity = similarity_calculator.calculate_similarity(
            oauth_doc, jwt_doc
        )
        oauth_mobile_similarity = similarity_calculator.calculate_similarity(
            oauth_doc, mobile_doc
        )
    ...
    async def test_error_handling_and_edge_cases(self, intelligence_engine):
        ...
        incomplete_analysis = intelligence_engine.analyze_document_relationships(
            [incomplete_doc]
        )
    ...
    @pytest.mark.asyncio
    async def test_memory_efficiency_with_large_similarity_matrix(
        ...
        test_docs = base_docs[
            :6
        ]  # Use 6 docs = 15 pairwise comparisons (reduced from 8)
        ...
        ), f"Similarity calculation took too long: {calculation_time:.2f}s"
    ...
    async def test_concurrent_analysis_operations(self, intelligence_engine):
        ...
        async def run_clustering_analysis():
            ...
            cluster_analyzer = DocumentClusterAnalyzer(similarity_calculator)
            return cluster_analyzer.create_clusters(
                docs[:4], ClusteringStrategy.MIXED_FEATURES
            )
