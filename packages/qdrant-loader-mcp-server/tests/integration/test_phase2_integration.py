...
class TestPhase2Integration:
    ...
    def test_semantic_content_discovery(self):
        ...
        for query in test_queries:
            ...
            for i, result in enumerate(related_content[:3]):  # Show top 3
                ...
                print(
                    f"      • Reasoning: {result.reasoning_path[0] if result.reasoning_path else 'Direct match'}"
                )

            # Should find some related content for these queries
            if len(related_content) > 0:
                print("   ✅ Successfully discovered related content")
            else:
                print("   ℹ️ No related content found (may be expected for this query)")
    ...
    def test_multi_hop_reasoning(self):
        ...
        for strategy, name in strategies:
            ...
            results = self.knowledge_graph.find_related_content(
                query=test_query, max_hops=2, max_results=5, strategy=strategy
            )
    ...
    def test_cross_document_relationships(self):
        ...
        for node_type, nodes in nodes_by_type.items():
            ...
            sorted_nodes = sorted(
                nodes, key=lambda n: n.get("centrality", 0), reverse=True
            )
            for i, node in enumerate(sorted_nodes[:2]):
                print(
                    f"     {i+1}. '{node['title']}' (centrality: {node.get('centrality', 0):.3f})"
                )
        ...
        for rel_type, edges in relationships_by_type.items():
            ...
            for i, edge in enumerate(edges[:2]):
                ...
                print(
                    f"     {i+1}. '{source_node.get('title', 'Unknown')}' → '{target_node.get('title', 'Unknown')}' (weight: {edge['weight']:.2f})"
                )
        ...
        if cross_doc_relationships:
            ...
            print(
                "ℹ️ No cross-document relationships found (may need more diverse content)"
            )
    ...
    def test_performance_benchmarks(self):
        ...
        for _i in range(3):  # Run multiple times for average
            start_time = time.time()
            self.knowledge_graph.build_graph(self.sample_search_results)
        ...
        print(
            f"   🎯 Target: < 1000ms ({'✅ PASS' if avg_construction_time < 1000 else '❌ FAIL'})"
        )
        ...
        for query in test_queries:
            start_time = time.time()
            results = self.knowledge_graph.find_related_content(
                query=query, max_hops=2, max_results=5
            )
            query_time = (time.time() - start_time) * 1000
        ...
        print(
            f"   🎯 Target: < 100ms ({'✅ PASS' if avg_query_time < 100 else '❌ FAIL'})"
        )
        ...
        print(
            f"   🎯 Efficiency ratio: {edges_per_result/nodes_per_result:.2f} edges/node"
        )
    ...
    def test_integration_with_spacy_phase1(self):
        ...
        if semantic_results:
            avg_semantic_score = sum(r.semantic_score for r in semantic_results) / len(
                semantic_results
            )
    ...
    def test_complete_phase2_workflow(self):
        ...
        for strategy in strategies:
            ...
            results = self.knowledge_graph.find_related_content(
                query=test_query, strategy=strategy, max_hops=3, max_results=8
            )
            ...
            print(
                f"   🔄 {strategy.value}: {len(results)} results in {step2_time:.2f}ms"
            )
        ...
        stats = self.knowledge_graph.get_graph_statistics()
        self.knowledge_graph.export_graph(format="json")
        ...
        performance_metrics = {
            ...
            "avg_query_time": sum(r["time"] for r in discovery_results.values())
        ...
        print("\n📊 Performance Summary:")
        print(
            f"   ⏱️ Total workflow: {performance_metrics['total_workflow_time']:.2f}ms"
        )
        print(
            f"   🏗️ Graph construction: {performance_metrics['graph_construction_time']:.2f}ms"
        )
        ...
        print(
            f"   🎯 Scalability: {performance_metrics['nodes_created']} nodes, {performance_metrics['relationships_created']} edges"
        )

        # Validate performance targets
        assert performance_metrics["total_workflow_time"] < 5000  # < 5 seconds total
        assert (
            performance_metrics["graph_construction_time"] < 2000
        )  # < 2 seconds construction
        ...
        assert (
            performance_metrics["relationships_created"] > 0
...
if __name__ == "__main__":
    ...
    print("🏆 All tests passed with excellent performance metrics!")
