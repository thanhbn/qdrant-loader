...
class TestAsyncSearchBehavior:
    ...
    @pytest.mark.asyncio
    async def test_search_async_execution_order(
    ...
    async def test_concurrent_search_requests(
        ...
        with patch.object(
            ...
            with patch.object(async_search_handler.formatters, "format_search_result"):
                ...
                assert (
                    async_search_handler.query_processor.process_query.call_count == 5
                )
    ...
    @pytest.mark.asyncio
    async def test_search_with_slow_query_processor(
        self, async_search_handler, sample_async_results
    ):
    ...
    @pytest.mark.asyncio
    async def test_search_with_slow_search_engine(
        self, async_search_handler, sample_async_results
    ):
        """Test search behavior when search engine is slow."""
        async_search_handler.query_processor.process_query.return_value = {
            "query": "fast_processed"
        }
...
class TestAsyncHierarchySearchBehavior:
    ...
    @pytest.mark.asyncio
    async def test_hierarchy_search_async_filtering(
        self, async_search_handler, sample_async_results
    ):
        """Test async behavior with hierarchy filtering."""
        async_search_handler.query_processor.process_query.return_value = {
            "query": "hierarchy_test"
        }
        async_search_handler.search_engine.search.return_value = sample_async_results
        async_search_handler.protocol.create_response.return_value = {
            "jsonrpc": "2.0",
        ...
        with patch.object(
            async_search_handler, "_apply_hierarchy_filters", side_effect=slow_filter
        ):
            with patch.object(
                async_search_handler.formatters, "create_lightweight_hierarchy_results"
            ):
                start_time = time.time()
                params = {
    ...
    @pytest.mark.asyncio
    async def test_concurrent_hierarchy_searches(
        self, async_search_handler, sample_async_results
    ):
        """Test concurrent hierarchy search requests."""
        async_search_handler.query_processor.process_query.return_value = {
            "query": "concurrent_hierarchy"
        }
        async_search_handler.search_engine.search.return_value = sample_async_results
        async_search_handler.protocol.create_response.return_value = {"jsonrpc": "2.0"}
...
class TestAsyncAttachmentSearchBehavior:
    ...
    @pytest.mark.asyncio
    async def test_attachment_search_async_organization(
        self, async_search_handler, sample_async_results
    ):
        """Test async behavior with attachment organization."""
        async_search_handler.query_processor.process_query.return_value = {
            "query": "attachment_test"
        }
        async_search_handler.search_engine.search.return_value = sample_async_results
        async_search_handler.protocol.create_response.return_value = {
            "jsonrpc": "2.0",
        ...
        with patch.object(
            async_search_handler.formatters, "_organize_attachments_by_type"
        ) as mock_organize:
            ...
            with patch.object(
                async_search_handler.formatters, "create_lightweight_attachment_results"
            ):
                time.time()
                params = {
    ...
    @pytest.mark.asyncio
    async def test_concurrent_attachment_searches(
        self, async_search_handler, sample_async_results
    ):
        """Test concurrent attachment search requests."""
        async_search_handler.query_processor.process_query.return_value = {
            "query": "concurrent_attachment"
        }
        async_search_handler.search_engine.search.return_value = sample_async_results
        async_search_handler.protocol.create_response.return_value = {"jsonrpc": "2.0"}
        ...
        with patch.object(
            ...
            with patch.object(
                ...
                for i in range(3):
                    ...
                    task = async_search_handler.handle_attachment_search(
                        f"attach_{i}", params
                    )
                    tasks.append(task)
        ...
        with patch.object(async_search_handler.formatters, "format_search_result"):
            with patch.object(
                ...
                for i in range(4):
                    ...
                    task = async_search_handler.handle_expand_document(
                        f"expand_{i}", params
                    )
                    tasks.append(task)
    ...
    async def test_async_timeout_handling(self, async_search_handler):
        ...
        with pytest.raises(asyncio.TimeoutError):
            await asyncio.wait_for(
                async_search_handler.handle_search(1, params), timeout=0.1
            )
...
class TestAsyncPerformance:
    ...
    @pytest.mark.asyncio
    async def test_memory_efficiency_concurrent_operations(
        self, async_search_handler, sample_async_results
    ):
    ...
    @pytest.mark.asyncio
    async def test_async_operation_isolation(
        self, async_search_handler, sample_async_results
    ):
        ...
        with patch.object(
            ...
            with patch.object(async_search_handler.formatters, "format_search_result"):
                ...
                for i in range(3):
                    ...
                    assert (
                        f"tracked_isolation_query_{i}" in call_tracking["search_calls"]
                    )
