...
class TestCrossDocumentIntelligenceAdditionalCoverage:
    ...
    def test_complementary_finder_knowledge_graph_integration(self, mock_spacy_analyzer):
        ...
        def mock_extract_entity_texts(entities):
            if not entities:
                return []
            return [e.get("text", "").lower() if isinstance(e, dict) else str(e).lower() for e in entities]
        
        def mock_extract_topic_texts(topics):
            if not topics:
                return []
            return [t.get("text", "").lower() if isinstance(t, dict) else str(t).lower() for t in topics]
        
        mock_similarity_calc._extract_entity_texts.side_effect = mock_extract_entity_texts
    ...
    @pytest.mark.asyncio
    async def test_conflict_detector_vector_similarity_integration(self, mock_spacy_analyzer):
