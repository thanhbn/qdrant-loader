...
async def test_generate_topic_chain_success(
    search_engine, qdrant_config, openai_config, mock_qdrant_client, mock_openai_client, mock_topic_chain
):
    ...
    with (
        patch("qdrant_loader_mcp_server.search.engine.AsyncQdrantClient", return_value=mock_qdrant_client),
        patch("qdrant_loader_mcp_server.search.engine.AsyncOpenAI", return_value=mock_openai_client),
        patch("qdrant_loader_mcp_server.search.engine.HybridSearchEngine", return_value=mock_hybrid_search),
    ):
        await search_engine.initialize(qdrant_config, openai_config)
        
        result = await search_engine.generate_topic_chain(
            query="AI and machine learning",
            strategy="mixed_exploration",
            max_links=5
        )
...
async def test_execute_topic_chain_success(
    ...
    mock_topic_chain, sample_search_results
):
...
async def test_search_with_facets_success(
    search_engine, qdrant_config, openai_config, mock_qdrant_client, mock_openai_client, sample_search_results
):
...
async def test_get_facet_suggestions_success(
    ...
    mock_hybrid_search.suggest_facet_refinements = MagicMock(return_value=mock_suggestions)
...
async def test_analyze_document_relationships_insufficient_docs(
    ...
    mock_hybrid_search = AsyncMock()
    mock_hybrid_search.search.return_value = [sample_search_results[0]]  # Only 1 document
...
async def test_cluster_documents_adaptive_strategy(
    ...
    with (
        ...
        with patch.object(search_engine, '_select_optimal_strategy', return_value='entity_based'):
            result = await search_engine.cluster_documents(
                query="test query",
                strategy="adaptive"
            )
...
async def test_topic_chain_generation_error_handling(
    ...
    mock_hybrid_search = AsyncMock()
    mock_hybrid_search.generate_topic_search_chain.side_effect = Exception("Generation failed")
...
async def test_topic_chain_execution_error_handling(
    ...
    mock_hybrid_search = AsyncMock()
    mock_hybrid_search.execute_topic_chain_search.side_effect = Exception("Execution failed")
...
async def test_initialization_client_none_error(
    ...
    with (
        patch("qdrant_loader_mcp_server.search.engine.AsyncQdrantClient", return_value=None),
        patch("qdrant_loader_mcp_server.search.engine.AsyncOpenAI", return_value=mock_openai_client),
    ):
