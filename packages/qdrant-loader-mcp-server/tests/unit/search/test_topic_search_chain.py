...
class TestTopicRelationshipMap:
    ...
    def test_find_semantic_related_topics(self, topic_map, mock_spacy_analyzer):
        ...
        related = topic_map._find_semantic_related_topics(
            "authentication", max_related=3
        )
    ...
    def test_find_related_topics_combined(self, topic_map, sample_search_results):
        ...
        [topic for topic, score, rel_type in related]
        assert any(
            rel_type == "semantic_similarity" for topic, score, rel_type in related
        )
...
class TestTopicSearchChainGenerator:
    ...
    def test_initialize_from_results(
        self, chain_generator, sample_search_results_for_init
    ):
    ...
    def test_extract_primary_topics(self, chain_generator, mock_spacy_analyzer):
        ...
        primary_topics = chain_generator._extract_primary_topics(
            mock_spacy_analyzer.analyze_query_semantic(query), query
        )
    ...
    def test_generate_breadth_first_chain(
        self, chain_generator, sample_search_results_for_init
    ):
        ...
        # Mock related topics
        chain_generator.topic_map.find_related_topics = Mock(
            return_value=[
        ...
        assert (
            "oauth" in chain_links[0].related_topics
            or "jwt" in chain_links[0].related_topics
        )

    def test_generate_depth_first_chain(
        self, chain_generator, sample_search_results_for_init
    ):
    ...
    def test_generate_search_chain_mixed_exploration(
        self, chain_generator, sample_search_results_for_init
    ):
        ...
        topic_chain = chain_generator.generate_search_chain(
            original_query=query, strategy=ChainStrategy.MIXED_EXPLORATION, max_links=4
        )
    ...
    def test_calculate_discovery_potential(self, chain_generator):
        ...
        mock_analysis = Mock()
        potential = chain_generator._calculate_discovery_potential(
            chain_links, mock_analysis
        )
    ...
    def test_calculate_chain_coherence(self, chain_generator):
        ...
        incoherent_coherence = chain_generator._calculate_chain_coherence(
            incoherent_links
        )
...
class TestTopicChainIntegration:
    ...
    def test_end_to_end_topic_chain_generation(self, real_spacy_analyzer):
        ...
        for link in chain.chain_links:
            ...
            assert link.exploration_type in [
                "related",
                "deeper",
                "broader",
                "alternative",
    ...
    def test_topic_similarity_with_real_spacy(self, real_spacy_analyzer):
        ...
        TopicRelationshipMap(real_spacy_analyzer)
        ...
        assert (
            similarity > 0.1
        )  # Should be somewhat similar concepts (spaCy similarity varies)
