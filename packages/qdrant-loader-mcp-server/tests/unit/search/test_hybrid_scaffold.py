...
from qdrant_loader_mcp_server.search.hybrid.models import HybridStage, HybridWeights
...
async def test_hybrid_pipeline_runs_empty():
    pipe = HybridPipeline(vector_searcher=_DummyVector(), keyword_searcher=_DummyKeyword(), result_combiner=_DummyCombiner())
