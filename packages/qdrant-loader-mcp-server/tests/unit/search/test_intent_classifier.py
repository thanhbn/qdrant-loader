...
class TestIntentClassifier:
    ...
    def test_classify_business_intent(self, intent_classifier, mock_spacy_analyzer):
        ...
        assert not result.is_technical
    ...
    def test_classify_procedural_intent(self, intent_classifier, mock_spacy_analyzer):
        ...
        mock_spacy_analyzer.analyze_query_semantic.return_value = QueryAnalysis(
            ...
            pos_patterns=[
                ...
                "NOUN",
                "NOUN",
            ],  # "How to install and setup the development environment"
    ...
    def test_behavioral_context_weighting(self, intent_classifier, mock_spacy_analyzer):
        ...
        result2 = intent_classifier.classify_intent(
            query, behavioral_context=behavioral_context
        )
    ...
    def test_cache_management(self, intent_classifier):
        ...
        intent_classifier._intent_cache["test2"] = SearchIntent(
            IntentType.TECHNICAL_LOOKUP, 0.8
        )
    ...
    def test_fallback_on_error(self, intent_classifier, mock_spacy_analyzer):
        ...
        mock_spacy_analyzer.analyze_query_semantic.side_effect = Exception(
            "spaCy error"
        )
...
class TestAdaptiveSearchStrategy:
    ...
    def test_technical_lookup_config(self, adaptive_strategy):
        ...
        assert config.use_knowledge_graph
    ...
    def test_troubleshooting_config(self, adaptive_strategy):
        ...
        assert not config.expand_query  # Don't expand error queries
    ...
    def test_confidence_adjustments(self, adaptive_strategy):
        ...
        low_confidence_intent = SearchIntent(
            intent_type=IntentType.TECHNICAL_LOOKUP, confidence=0.3
        )
        ...
        assert (
            config_low.expansion_aggressiveness <= base_config.expansion_aggressiveness
        )
        assert config_low.diversity_factor >= base_config.diversity_factor

        # High confidence intent
        high_confidence_intent = SearchIntent(
            intent_type=IntentType.TECHNICAL_LOOKUP, confidence=0.9
        )

        config_high = adaptive_strategy.adapt_search(
            high_confidence_intent, "test query"
        )

        # Should increase precision for high confidence
        assert (
            config_high.expansion_aggressiveness >= base_config.expansion_aggressiveness
        )
    ...
    def test_secondary_intent_blending(self, adaptive_strategy):
        ...
        assert (
            abs(config.vector_weight - base_config.vector_weight) >= 0.001
            or config.vector_weight == base_config.vector_weight
        )
    ...
    def test_query_adaptations(self, adaptive_strategy):
        ...
        adaptive_strategy.adapt_search(search_intent, long_query)
        ...
        assert config_complex.use_knowledge_graph
    ...
    def test_strategy_stats(self, adaptive_strategy):
        ...
        assert stats["has_knowledge_graph"]
...
class TestAdaptiveSearchConfig:
    ...
    def test_default_config(self):
        ...
        assert not config.use_knowledge_graph
        assert config.expand_query
