    ...
    return [
        {
            ...
            "url": "http://example.com/doc2",
            "source": "repo",
            "metadata": {"chunk_index": 1, "total_chunks": 2, "word_count": 300},
        },
        {
...
def adaptive_config_mock():
    ...
    mock_config.ranking_boosts = {
        "has_code_blocks": 1.2,
        "section_type": {"documentation": 1.1},
    }
...
class TestCombineResultsCore:
    ...
    @pytest.mark.asyncio
    async def test_combine_results_basic(
        self,
        result_combiner,
        ...
        with patch.object(
            result_combiner.metadata_extractor, "extract_all_metadata", return_value={}
        ):
    ...
    @pytest.mark.asyncio
    async def test_combine_results_with_overlapping_content(
        self,
        result_combiner,
        sample_vector_results,
        sample_keyword_results,
        ...
        overlapping_result = next(
            (r for r in combined if r.document_id == "doc2"), None
        )
    ...
    @pytest.mark.asyncio
    async def test_combine_results_empty_inputs(
        self, result_combiner, sample_query_context
    ):
    ...
    @pytest.mark.asyncio
    async def test_combine_results_vector_only(
        self, result_combiner, sample_vector_results, sample_query_context
    ):
    ...
    @pytest.mark.asyncio
    async def test_combine_results_keyword_only(
        self, result_combiner, sample_keyword_results, sample_query_context
    ):
    ...
    @pytest.mark.asyncio
    async def test_combine_results_min_score_filtering(
        self, result_combiner, sample_query_context
    ):
    ...
    @pytest.mark.asyncio
    async def test_combine_results_with_limit(
        self,
        result_combiner,
        sample_vector_results,
        sample_keyword_results,
...
class TestShouldSkipResult:
    ...
    def test_should_skip_result_content_type_filtering_code(
        self, result_combiner, sample_query_context
    ):
        """Test content type filtering for code content."""
        metadata = {"content_type_analysis": {"has_code_blocks": True}}
    ...
    def test_should_skip_result_content_type_filtering_documentation(
        self, result_combiner, sample_query_context
    ):
        """Test content type filtering for documentation content."""
        metadata = {"content_type_analysis": {"has_code_blocks": False}}
    ...
    def test_should_skip_result_content_type_mismatch(
        self, result_combiner, sample_query_context
    ):
...
class TestBoostScoreWithMetadata:
    ...
    def test_boost_score_basic(self, result_combiner, sample_query_context):
        ...
        base_score = 0.7
        metadata = {"word_count": 500, "section_level": 2}
    ...
    def test_boost_score_with_intent_config(
        self, result_combiner, sample_query_context, adaptive_config_mock
    ):
        ...
        base_score = 0.6
        metadata = {"has_code_blocks": True, "source_type": "confluence"}
    ...
    def test_boost_score_with_spacy_analyzer(
        self, result_combiner_with_spacy, sample_query_context
    ):
        """Test score boosting with spaCy analyzer."""
        query_context_with_spacy = {
            **sample_query_context,
            "spacy_analysis": MagicMock(
                entities=["OAuth"], main_concepts=["authentication"]
            ),
        }
        ...
        result_combiner_with_spacy.spacy_analyzer.semantic_similarity_matching.return_value = (
            0.8
        )
    ...
    def test_boost_score_fallback_semantic(self, result_combiner, sample_query_context):
        ...
        base_score = 0.6
        metadata = {"entities": ["OAuth", "API"], "topics": ["authentication"]}
...
class TestApplyBoostingMethods:
    ...
    def test_apply_content_type_boosting(self, result_combiner, sample_query_context):
        ...
        boost = result_combiner._apply_content_type_boosting(
            metadata, query_context_code
        )
        assert boost > 0
    ...
    def test_apply_conversion_boosting(self, result_combiner, sample_query_context):
        ...
        # Test converted file
        metadata_converted = {"is_converted": True, "original_file_type": "docx"}
        boost = result_combiner._apply_conversion_boosting(
            metadata_converted, sample_query_context
        )
        assert boost == 0.08

        # Test Excel sheet with data query
        metadata_excel = {"is_excel_sheet": True}
        query_context_data = {**sample_query_context, "keywords": ["data", "table"]}
        boost = result_combiner._apply_conversion_boosting(
            metadata_excel, query_context_data
        )
        assert boost == 0.12

    def test_apply_fallback_semantic_boosting(
        self, result_combiner, sample_query_context
    ):
        ...
        boost = result_combiner._apply_fallback_semantic_boosting(
            metadata, sample_query_context
        )
...
class TestApplyDiversityFiltering:
    ...
    def test_apply_diversity_filtering_with_diversity(self, result_combiner):
        ...
        results = [
            create_hybrid_search_result(
                0.9, "text1", "git", "title1", section_type="code"
            ),
            create_hybrid_search_result(
                0.8, "text2", "git", "title2", section_type="code"
            ),
            create_hybrid_search_result(
                0.7, "text3", "confluence", "title3", section_type="docs"
            ),
            create_hybrid_search_result(
                0.6, "text4", "jira", "title4", section_type="issue"
            ),
        ]
...
class TestFlattenMetadataComponents:
    ...
    def test_flatten_metadata_components_with_none(self, result_combiner):
        ...
        components = {"component1": None, "component2": {"key": "value"}}
...
class TestEdgeCasesAndErrorHandling:
    ...
    @pytest.mark.asyncio
    async def test_combine_results_with_malformed_data(
        self, result_combiner, sample_query_context
    ):
    ...
    @pytest.mark.asyncio
    async def test_combine_results_with_none_metadata(
        self, result_combiner, sample_query_context
    ):
    ...
    def test_boost_score_with_empty_metadata(
        self, result_combiner, sample_query_context
    ):
