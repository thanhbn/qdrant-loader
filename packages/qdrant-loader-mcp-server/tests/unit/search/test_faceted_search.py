...
class TestDynamicFacetGenerator:
    ...
    def test_generate_content_type_facets(self):
        ...
        assert any(
            v.value == "confluence" and v.count == 2 for v in content_facet.values
        )
        assert any(
            v.value == "localfile" and v.count == 1 for v in content_facet.values
        )
    ...
    def test_generate_features_facets(self):
        ...
        tables_value = next(
            (v for v in features_facet.values if v.value == "tables"), None
        )
    ...
    def test_generate_project_facets(self):
        ...
        facets = generator.generate_facets(results)
        project_facet = next(
            (f for f in facets if f.facet_type == FacetType.PROJECT), None
        )

        assert project_facet is not None
        assert len(project_facet.values) == 2
        assert any(
            v.value == "MyaHealth" and v.count == 2 for v in project_facet.values
        )
    ...
    def test_generate_entity_facets(self):
        ...
        facets = generator.generate_facets(results)
        entity_facet = next(
            (f for f in facets if f.facet_type == FacetType.ENTITIES), None
        )
    ...
    def test_generate_hierarchy_depth_facets(self):
        ...
        facets = generator.generate_facets(results)
        depth_facet = next(
            (f for f in facets if f.facet_type == FacetType.HIERARCHY_DEPTH), None
        )
        ...
        shallow_value = next(
            (v for v in depth_facet.values if v.value == "shallow"), None
        )
    ...
    def test_facet_display_names(self):
        ...
        results = [
            self._create_search_result(has_code_blocks=True, estimated_read_time=1)
        ]
        facets = generator.generate_facets(results)
        ...
        if features_facet:
            code_value = next(
                (v for v in features_facet.values if v.value == "code"), None
            )
            assert code_value is not None
            assert code_value.display_name == "Code Blocks"

        read_time_facet = next(
            (f for f in facets if f.facet_type == FacetType.READ_TIME), None
        )
        if read_time_facet:
            quick_value = next(
                (v for v in read_time_facet.values if v.value == "quick"), None
            )
    ...
    def test_facet_priority_sorting(self):
        ...
        content_type_index = next(
            (i for i, f in enumerate(facets) if f.facet_type == FacetType.CONTENT_TYPE),
            -1,
        )
        entities_index = next(
            (i for i, f in enumerate(facets) if f.facet_type == FacetType.ENTITIES), -1
        )
...
class TestFacetFilter:
    ...
    def test_content_type_filter_matches(self):
        ...
        result2 = create_hybrid_search_result(
            score=0.8, text="test", source_type="localfile", source_title="Test"
        )
    ...
    def test_features_filter_matches(self):
        ...
        filter_obj = FacetFilter(
            facet_type=FacetType.HAS_FEATURES, values=["code", "tables"], operator="OR"
        )
    ...
    def test_and_operator_filter(self):
        ...
        filter_obj = FacetFilter(
            facet_type=FacetType.HAS_FEATURES, values=["code", "tables"], operator="AND"
        )
    ...
    def test_entity_filter_matches(self):
        ...
        filter_obj = FacetFilter(
            facet_type=FacetType.ENTITIES, values=["oauth"], operator="OR"
        )
...
class TestFacetedSearchEngine:
    ...
    def test_apply_single_filter(self):
        ...
        results = [
            create_hybrid_search_result(
                score=0.8, text="test1", source_type="confluence", source_title="Test1"
            ),
            create_hybrid_search_result(
                score=0.8, text="test2", source_type="localfile", source_title="Test2"
            ),
            create_hybrid_search_result(
                score=0.8, text="test3", source_type="confluence", source_title="Test3"
            ),
        ]
    ...
    def test_suggest_refinements(self):
        ...
        results = [
            ...
            create_hybrid_search_result(
                score=0.8, text="test2", source_type="confluence", source_title="Test2"
            ),
            create_hybrid_search_result(
                score=0.8, text="test3", source_type="localfile", source_title="Test3"
            ),
        ]
...
class TestFacetedSearchIntegration:
    ...
    def test_complete_faceted_search_workflow(self):
        ...
        results = [
            ...
            create_hybrid_search_result(
                ...
                project_name="MyaHealth",
                has_tables=True,
                entities=[{"text": "PostgreSQL", "label": "PRODUCT"}],
                depth=1,
            ),
            create_hybrid_search_result(
                score=0.7,
        ...
        content_facet = next(
            (
                f
                for f in faceted_results.facets
                if f.facet_type == FacetType.CONTENT_TYPE
            ),
            None,
        )
        assert content_facet is not None
        ...
        if suggestions:
            ...
            content_suggestions = [
                s for s in suggestions if s.get("facet_type") == "content_type"
            ]
    ...
    def test_performance_with_large_dataset(self):
        ...
        filtered_results = engine.apply_facet_filters(results, filters)
        expected_count = len(
            [r for r in results if r.source_type == "confluence" and r.has_code_blocks]
        )
